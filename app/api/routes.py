...
from app.schemas.models import TodoItemCreate,TodoUpdate,TodoResponse
...
def replace_item(id:int,update_item: TodoUpdate,db:Session=Depends(get_db)):
    if id < 0:
        raise ValueError("Id cannot be negative")
    else:    
        if crud_todo.update_todo(db,id,update_item):
            return {"message" : f"Item {id} has been updated"}
